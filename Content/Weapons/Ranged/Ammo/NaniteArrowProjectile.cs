...
{
    public class NaniteArrowProjectile : ModProjectile
    {
        ...
        {
            SoundEngine.PlaySound(SoundID.Item132 with { Volume = 1f }, Projectile.Center);

            for (int i = 0; i < 25; i++)
            {
                Vector2 velocity = Vector2.One.RotatedBy(MathHelper.TwoPi * (i / 25f));

                Dust.NewDustPerfect(Projectile.Center, DustID.DrillContainmentUnit, velocity * 2f, 0, default, 1f).noGravity = true;
            }
        ...
        {
            SoundEngine.PlaySound(SoundID.Item132 with { Volume = 0.7f }, Projectile.Center);
